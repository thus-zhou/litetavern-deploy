...
### 📧 邮件发送失败/无法注册怎么办？
由于云服务器 IP 可能被邮件服务商屏蔽（尤其是 QQ 邮箱），导致注册邮件发送失败。
我们为您提供了 **邀请码 (Invite Code)** 系统作为备用方案：

1. **管理员生成邀请码**:
   * 登录管理员账号 (默认 `admin` / `admin123`)。
   * 进入 **Admin Dashboard** -> **Shop & Safety**。
   * 在 "Registration Invite Codes" 区域生成邀请码 (例如 `INV-A1B2C3`)。
2. **用户使用邀请码注册**:
   * 用户在注册页面，如果收不到邮件，可以点击 "Have an Invite Code?"。
   * 直接输入邀请码，即可跳过邮箱验证完成注册。
